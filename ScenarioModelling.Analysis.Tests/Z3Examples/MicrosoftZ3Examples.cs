...
public class MicrosoftZ3Examples
{
    ...
    static void SMT2FileTest(string filename)
    {
        ...
        using (Context ctx = new Context(new Dictionary<string, string>() { { "MODEL", "true" } }))
        {
            ...
            Console.WriteLine("SMT2 file read time: " + (DateTime.Now - before).TotalSeconds + " sec");
        ...
        Console.WriteLine("SMT2 file test took " + (DateTime.Now - before).TotalSeconds + " sec");
    }
    ...
    public static void EnumExample(Context ctx)
    {
        ...
        Prove(ctx, ctx.MkNot((BoolExpr)ctx.MkApp(fruit.TesterDecls[0], orange)));
    ...
    public static void CheckSmall(Context ctx, Solver solver, BitVecExpr[] to_minimize)
    {
        ...
        uint[] upper = new uint[num_Exprs];
        uint[] lower = new uint[num_Exprs];
        BitVecExpr[] values = new BitVecExpr[num_Exprs];
        for (int i = 0; i < upper.Length; ++i)
        {
            upper[i] = uint.MaxValue;
            lower[i] = 0;
        }
        bool some_work = true;
        int last_index = -1;
