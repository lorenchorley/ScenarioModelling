...
{
    public static T EqualityTypeCases<T>(object left, object right, MetaState system, Func<State, string, T> compareStateAndString, Func<bool, bool, T> compareBools, Func<string, string, T> compareStrings, Func<Entity, Entity, T> compareEntities)
    {
        // Null checks
        if (left == null)
    ...
    public static void CheckEqualityTypeCases(Type? left, Type? right, MetaState system)
    {
        // Null checks
        if (left == null)
            throw new Exception("Left side of the expression is null");
